...
💡 *Tip:* You can type \`cancel\` anytime to stop an ongoing action!
  ...
  // -------------------------------------------------------------
  const pendingExpenseByChat = new Map<number, {
    step: number;
    data: {
      title?: string;
      category?: string;
  ...
  // -------------------------------------------------------------
  const pendingProductByChat = new Map<number, {
    step: number;
    data: {
      name?: string;
      category?: string;
  ...
  // -------------------------------------------------------------
  const pendingSaleByChat = new Map<number, {
    step: number;
    data: {
      productName?: string;
      quantity?: string;
  ...
  bot.onText(/\/date(?:\s+(.+))?/, async (msg, match) => {
    ...
    try {
      ...
      if (productsRes.data.length === 0) message += `   None\n`;
      else productsRes.data.forEach((p: any) => message += `   • ${p.name} (Stock: ${p.stock})\n`);

      message += `\n📈 *Sales Logged (${salesRes.total}):*\n`;
      if (salesRes.data.length === 0) message += `   None\n`;
      else salesRes.data.forEach((s: any) => message += `   • Sale #${s.id} - ₹${s.totalAmount}\n`);

      message += `\n📉 *Expenses Logged (${expensesRes.total}):*\n`;
      if (expensesRes.data.length === 0) message += `   None\n`;
      else expensesRes.data.forEach((e: any) => message += `   • ${e.title} - ₹${e.amount}\n`);
  ...
  bot.onText(/\/products(?:\s+(.+))?$/i, async (msg, match) => {
    ...
    if (param) {
      if (param.toLowerCase() === "asc") sortOrder = "asc";
      else if (param.toLowerCase() === "desc") sortOrder = "desc";
      else {
        const dayRange = reportService.getStartAndEndOfDay(param);
        if (dayRange) filterDate = dayRange.startDate.toISOString().split('T')[0];
      }
    }
  ...
  bot.onText(/\/profit(?:\s+(.+))?/, async (msg, match) => {
    ...
    if (param) {
      ...
      if (dayRange) {
        ...
        if (parts[0] && !isNaN(Number(parts[0]))) month = parseInt(parts[0]);
        if (parts[1] && !isNaN(Number(parts[1]))) year = parseInt(parts[1]);
        else if (month) year = new Date().getFullYear();
      }
    }
    ...
    if (query.data.startsWith('upload_')) {
      ...
      try {
        ...
        if (typeStr === "products") {
          ...
          if (expenses.length === 0) throw new Error("No valid expenses found.");
          ...
          if (salesToCreate.length === 0) throw new Error("No valid sales found (all rows had errors or missing products).");
        ...
        if (result.failed > 0) {
          ...
          if (result.errors.length > 10) replyMsg += `\n...and ${result.errors.length - 10} more.`;
        }
