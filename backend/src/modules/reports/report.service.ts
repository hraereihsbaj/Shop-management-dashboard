...
export function getStartAndEndOfDay(dateStr: string) {
...
export async function getProfitAndLoss(month?: number, year?: number, date?: string) {
  let dateFilter = {};
...
export async function getChartData(period: string) {
  const now = new Date();
  ...
  // Group by date
  const dataMap = new Map<string, { revenue: number, cogs: number, expense: number }>();
